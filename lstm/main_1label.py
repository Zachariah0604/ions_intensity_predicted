...
def parse_args():
    ...
    parser.add_argument('--num-iter', type=int, default=150,help='the number of training steps to take')
    ...
    parser.add_argument('--input-size', type=int, default=281,help='')
...
def train(args,data):
    ...
    with tf.Session() as sess:
        ...
        sess.run(init)
        #train_y,train_X,merge_train_list=get_train_data('data/data_swedcad_mm/am/train_seq_che_test.csv')
        _,_,train_X,train_y,merge_train_list=data.get_data('data/data_swedcad_mm/am/test_1label.txt')
        ...
        for Iter in range(args.num_iter):
            ...
            for i,(train_piptide_index) in enumerate(suffled_batch_peptide):
                ...
                for j in range(len(suffled_train_piptide_index)):
                    train_ion_index=get_split_list(suffled_train_piptide_index[j])
        ...
        print("SaveModel:",tf.train.Saver().save(sess,'lstm/model_1label/bilstm_model.ckpt'))
 
        
def MSE(label,pred):
    return tf.losses.huber_loss(pred,label,delta=0.3)
  
def model_predict(_batch_size,_keep_prob,data):
    print('predicting..')
    test_idx,peptide,test_X,test_y,merge_test_list=data.get_data('data/data_swedcad_mm/am/test_1label.txt')
    ...
    with tf.Session() as session:
        ...
        saver = tf.train.import_meta_graph('lstm/model_1label/bilstm_model.ckpt.meta')
        saver.restore(session, tf.train.latest_checkpoint('lstm/model_1label/'))
        ...
        for i,(test_piptide_index) in enumerate(batch_peptide):
            ...
            #if padding_pep_num >0:
            #    for k in range(padding_pep_num):
            #       X.append(padding_data(np.zeros((2,279)),1,_max_ions_number))
            ...
            pred_[pred_>1]=1
            pred_[pred_<0]=0 
            _mse=session.run(MSE(tf.reshape(y,[-1,1]),pred_))
...
def get_merge_pred(merge_list,pred,data):
    ...
    return merge_list
def calc_pear(test_idx,peptide,pred,merge_list,data):
    
    pred_pd=write_pred_1label(test_idx,peptide,pred)
    merge_list=get_merge_pred(merge_list,pred_pd,data)
   
    person_mean=get_pearson_1label(merge_list) 
    return person_mean

def test(_batch_size,_keep_prob,data):
    test_idx,peptide,pred,merge_test_list=model_predict(_batch_size,_keep_prob,data)
    person_mean=calc_pear(test_idx,peptide,pred,merge_test_list,data)
    print(person_mean)

def main(args):
    data=GetData(1)
    if args.is_train==1:
        model_train(args,data)
    elif args.is_train==2:
        test(args.batch_size,args.keep_prob,data)
    else:
        train(args,data)
        test(args.batch_size,args.keep_prob,data)
